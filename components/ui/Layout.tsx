...
import { 
  ...
  ClipboardList, Users, History, Key, BarChart3, Camera, Presentation
} from 'lucide-react';
    ...
    <div className={`min-h-screen flex ${isDarkMode ? 'dark' : ''} bg-slate-50 dark:bg-slate-900 transition-colors duration-300 font-sans`}>
      ...
      <aside className={`fixed inset-y-0 left-0 w-72 bg-white dark:bg-slate-850 border-r border-gray-200 dark:border-slate-700 z-50 transform transition-transform duration-300 md:translate-x-0 md:static md:block ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-6 flex flex-col items-center justify-center border-b border-gray-100 dark:border-slate-800">
           <div className="w-48 mb-2 text-center">
             <div className="text-2xl font-black tracking-tight text-slate-800 dark:text-white">
                HALA<span className="text-emerald-500">GEL</span>
             </div>
             <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Manufacturing System</p>
           </div>
        </div>

        <nav className="p-4 space-y-1 overflow-y-auto max-h-[calc(100vh-120px)] custom-scrollbar">
          <Link to="/" onClick={() => setIsMobileMenuOpen(false)} className={navItemClass('/')}>
            <LayoutDashboard className="w-4 h-4" /> Dashboard
          </Link>
          <Link to="/overview" onClick={() => setIsMobileMenuOpen(false)} className={navItemClass('/overview')}>
            <Presentation className="w-4 h-4" /> System Overview
          </Link>
          ...
          {user && (
            <>
              ...
              <button onClick={() => { setShowAvatarModal(true); setIsMobileMenuOpen(false); }} className="w-full flex items-center gap-3 px-4 py-3 text-sm font-bold text-slate-500 hover:bg-gray-50 dark:hover:bg-slate-800 rounded-xl transition">
                <Camera className="w-4 h-4 text-emerald-500" /> Profile Picture
              </button>
